...
local function BNCaller(pp)--helps me log better
	local BNsss,BNerr=pcall(pp)
	if not BNsss then warn("BN script error: "..BNerr) end
end

BNCaller(function() getgenv().RealNamelessLoaded=true end)
BNCaller(function() getgenv().BNTestingVer=false end)
BNCaller(function() getgenv().cdshkjvcdsojuefdwonjwojgrwoijuhegr="FIWUIUR" end)
...
local BNSCREENGUI=nil --Getmodel("rbxassetid://140418556029404")
local BNjson = nil
local cmd={}
local BNmanage={}
local BNImageAssets = {
...
BNSESSIONSTARTEDIDK = os.clock()
lib={}
gui={}
BNCOLOREDELEMENTS={}
cmdBNnum=0
BNQoTEnabled = nil
BNiconSaveEnabled = nil
BNUISTROKER = Color3.fromRGB(148, 93, 255)

BNCaller(function() while not BNjson do Wait(.1) BNjson=HttpService:JSONDecode(game:HttpGet("https://raw.githubusercontent.com/ltseverydayyou/uuuuuuu/refs/heads/main/BN%20stuff.json")) end end)
...
function BNProtection(inst,var)
...
BNmanage.guiCHECKINGAHHHHH=function()
...
--[[ Version ]]--
local curVer = isAprilFools() and Format("%d.%d.%d", math.random(1, 99), math.random(0, 99), math.random(0, 99)) or BNjson.ver

--[[ Brand ]]--
local mainName = 'Nameless Admin'
local testingName = 'BN Testing'
local adminName = 'BN'
...
if not gethui then
	getgenv().gethui=function()
		return BNmanage.guiCHECKINGAHHHHH()
	end
end
...
local opt={
	prefix=prefixCheck;
	BNupdDate='unknown'; --month,day,year
	githubUrl = '';
	loader='';
	BNUILOADER='';
	BNAUTOSCALER=nil;
	BN_storage=nil;--Stupid Ahh script removing folders
	BNREQUEST = request or http_request or (syn and syn.request) or function() end;
...
if getgenv().BNTestingVer then
	opt.loader=[[loadstring(game:HttpGet("https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/main/BN%20testing.lua"))();]]
	opt.githubUrl="https://api.github.com/repos/ltseverydayyou/Nameless-Admin/commits?path=BN%20testing.lua"
	opt.BNUILOADER="https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/refs/heads/main/BNUITEST.lua"
else
	...
	opt.BNUILOADER="https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/refs/heads/main/BNUI.lua"
end
...
local BNfiles = {
	BNFILEPATH = "Nameless-Admin";
	BNWAYPOINTFILEPATH = "Nameless-Admin/Waypoints";
	BNPLUGINFILEPATH = "Nameless-Admin/Plugins";
	BNASSETSFILEPATH = "Nameless-Admin/Assets";
	BNPREFIXPATH = "Nameless-Admin/Prefix.txt";
	BNBUTTONSIZEPATH = "Nameless-Admin/ButtonSize.txt";
	BNUISIZEPATH = "Nameless-Admin/UIScale.txt";
	BNQOTPATH = "Nameless-Admin/QueueOnTeleport.txt";
	BNALIASPATH = "Nameless-Admin/Aliases.json";
	BNICONPOSPATH = "Nameless-Admin/IconPosition.json";
	BNUSERBUTTONSPATH = "Nameless-Admin/UserButtons.json";
	BNAUTOEXECPATH = "Nameless-Admin/AutoExecCommands.json";
	BNPREDICTIONPATH = "Nameless-Admin/Prediction.txt";
	BNSTROKETHINGY = "Nameless-Admin/BNUIStroker.txt";
	BNJOINLEAVE = "Nameless-Admin/JoinLeave.json";
	BNJOINLEAVELOG = "Nameless-Admin/JoinLeaveLog.txt";
	BNCHATTAG = "Nameless-Admin/ChatTag.json";
}
BNUserButtons = {}
UserButtonGuiList = {}
BNEXECDATA = BNEXECDATA or {commands = {}, args = {}}
doPREDICTION = true

-- Creates folder & files for Prefix, Plugins, and etc
if FileSupport then
	if not isfolder(BNfiles.BNFILEPATH) then
		makefolder(BNfiles.BNFILEPATH)
	end

	if not isfolder(BNfiles.BNWAYPOINTFILEPATH) then
		makefolder(BNfiles.BNWAYPOINTFILEPATH)
	end

	if not isfolder(BNfiles.BNPLUGINFILEPATH) then
		makefolder(BNfiles.BNPLUGINFILEPATH)
	end

	if not isfolder(BNfiles.BNASSETSFILEPATH) then
		makefolder(BNfiles.BNASSETSFILEPATH)
	end

	if not isfile(BNfiles.BNPREFIXPATH) then
		writefile(BNfiles.BNPREFIXPATH, ";")
	end

	if not isfile(BNfiles.BNBUTTONSIZEPATH) then
		writefile(BNfiles.BNBUTTONSIZEPATH, "1")
	end

	if not isfile(BNfiles.BNUISIZEPATH) then
		writefile(BNfiles.BNUISIZEPATH, "1")
	end

	if not isfile(BNfiles.BNQOTPATH) then
		writefile(BNfiles.BNQOTPATH, "false")
	end

	if not isfile(BNfiles.BNALIASPATH) then
		writefile(BNfiles.BNALIASPATH, "{}")
	end

	if not isfile(BNfiles.BNICONPOSPATH) then
		writefile(BNfiles.BNICONPOSPATH, HttpService:JSONEncode({
			X = 0.5;
			Y = 0.1;
			Save = false;
		}))
	...
	if not isfile(BNfiles.BNUSERBUTTONSPATH) then
		writefile(BNfiles.BNUSERBUTTONSPATH, HttpService:JSONEncode({}))
	end

	if not isfile(BNfiles.BNAUTOEXECPATH) then
		writefile(BNfiles.BNAUTOEXECPATH, "[]")
	end

	if not isfile(BNfiles.BNPREDICTIONPATH) then
		writefile(BNfiles.BNPREDICTIONPATH, "true")
	end

	if not isfile(BNfiles.BNSTROKETHINGY) then
		writefile(BNfiles.BNSTROKETHINGY, HttpService:JSONEncode({
			R = 148 / 255;
			G = 93 / 255;
			B = 255 / 255;
		}))
	...
	if not isfile(BNfiles.BNJOINLEAVE) then
		writefile(BNfiles.BNJOINLEAVE, HttpService:JSONEncode({
			JoinLog = false;
			LeaveLog = false;
			SaveLog = false;
		}))
	...
	if not isfile(BNfiles.BNCHATTAG) then
		writefile(BNfiles.BNCHATTAG, HttpService:JSONEncode({
			Text = "Tag";
			Color = {
				R = 0;
				G = 1;
...
if FileSupport then
	prefixCheck = readfile(BNfiles.BNPREFIXPATH)
	BNsavedScale = tonumber(readfile(BNfiles.BNBUTTONSIZEPATH))
	BNUISavedScale = tonumber(readfile(BNfiles.BNUISIZEPATH))
	BNQoTEnabled = readfile(BNfiles.BNQOTPATH) == "true"
	doPREDICTION = readfile(BNfiles.BNPREDICTIONPATH) == "true"
	BNUISTROKER = InitUIStroke(BNfiles.BNSTROKETHINGY)
	...
	if BNsavedScale and BNsavedScale > 0 then
		BNScale = BNsavedScale
	else
		BNScale = 1
		writefile(BNfiles.BNBUTTONSIZEPATH, "1")
	...
	if BNUISavedScale and BNUISavedScale > 0 then
		BNUIScale = BNUISavedScale
	else
		BNUIScale = 1
		writefile(BNfiles.BNUISIZEPATH, "1")
	...
	if isfile(BNfiles.BNJOINLEAVE) then
		local success, data = pcall(function()
			return HttpService:JSONDecode(readfile(BNfiles.BNJOINLEAVE))
		end)
	...
	if isfile(BNfiles.BNCHATTAG) then
		local success, data = pcall(function()
			return HttpService:JSONDecode(readfile(BNfiles.BNCHATTAG))
		end)
	...
	if isfile(BNfiles.BNICONPOSPATH) then
		local success, data = pcall(function()
			return HttpService:JSONDecode(readfile(BNfiles.BNICONPOSPATH))
		end)
		if success and data then
			if data.Save ~= nil then
				BNiconSaveEnabled = data.Save
			else
				BNiconSaveEnabled = false
			end
		else
			BNiconSaveEnabled = false
		end
...
if opt.saveTag then
	SafeGetService("Players").LocalPlayer:SetAttribute("CustomBNtaggerText", opt.currentTagText)
	SafeGetService("Players").LocalPlayer:SetAttribute("CustomBNtaggerColor", opt.currentTagColor)
end

BNCaller(function()
	local response = opt.BNREQUEST({
		Url = opt.githubUrl,
		Method = "GET"
	})
...
BNCaller(function()
	...
	if type(BNImageAssets) ~= "table" then return end

	local baseURL = "https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/main/BNimages/"
	for _, fileName in pairs(BNImageAssets) do
		local fullPath = BNfiles.BNASSETSFILEPATH.."/"..fileName
		if not isfile(fullPath) then
			...
			if success and data then
				...
				warn("[BN] Failed to download:", fileName)
			end
		end
	end
end)
...
function isRelAdmin(Player)
	for _, id in ipairs(_G.BNadminsLol) do
...
LocalPlayer.OnTeleport:Connect(function(...)
	if BNQoTEnabled and opt.queueteleport then
...
opt.BN_storage = InstanceNew("ScreenGui")
NaProtectUI(opt.BN_storage)
...
function BNESP(player, persistent)
...
BNmanage.readAliasFile = function()
	if FileSupport and isfile(BNfiles.BNALIASPATH) then
		local success, data = pcall(function()
			return HttpService:JSONDecode(readfile(BNfiles.BNALIASPATH))
		end)
		if success and type(data) == "table" then
			return data
		end
...
BNmanage.loadAliases = function()
	local aliasMap = BNmanage.readAliasFile()
	for alias, original in pairs(aliasMap) do
		if cmds.Commands[original:lower()] then
			...
			cmds.BNSAVEDALIASES[alias:lower()] = true
		end
	end
end

BNmanage.loadButtonIDS = function()
	if FileSupport and isfile(BNfiles.BNUSERBUTTONSPATH) then
		local success, data = pcall(function()
			return HttpService:JSONDecode(readfile(BNfiles.BNUSERBUTTONSPATH))
		end)
		if success and type(data) == "table" then
			BNUserButtons = data
		end
...
BNmanage.loadAutoExec = function()
	BNEXECDATA = {commands = {}, args = {}}

	if FileSupport and isfile(BNfiles.BNAUTOEXECPATH) then
		local success, decoded = pcall(function()
			return HttpService:JSONDecode(readfile(BNfiles.BNAUTOEXECPATH))
		end)
		if success and type(decoded) == "table" then
			BNEXECDATA = decoded

			if not BNEXECDATA.commands then
				BNEXECDATA.commands = {}
			end
			if not BNEXECDATA.args then
				BNEXECDATA.args = {}
			end
		end
	end
...
BNmanage.LoadPlugins = function()
	...
	local files = listfiles(BNfiles.BNPLUGINFILEPATH)
...
BNmanage.RenderUserButtons = function()
	...
	local totalButtons = #BNUserButtons
	local totalWidth = totalButtons * (100 + 10)
	local startX = 0.5 - (totalWidth / 2) / BNSCREENGUI.AbsoluteSize.X
	local spacing = 110
	local TOGGLE_COLOR_ON = Color3.fromRGB(0, 170, 0)
	local TOGGLE_COLOR_OFF = Color3.fromRGB(30, 30, 30)

	local index = 0
	for id, data in pairs(BNUserButtons) do
		local btn = InstanceNew("TextButton")
		btn.Name = "BNUserButton_"..id
		btn.Text = data.Label
		btn.Size = UDim2.new(0, 60, 0, 60)
		btn.AnchorPoint = Vector2.new(0.5, 1)
		btn.Position = UDim2.new(startX + (spacing * index) / BNSCREENGUI.AbsoluteSize.X, 0, 0.9, 0)
		btn.Parent = BNSCREENGUI
		...
		if requiresArgs then
			...
			MouseButtonFix(saveToggle, function()
				...
				if FileSupport then
					writefile(BNfiles.BNUSERBUTTONSPATH, HttpService:JSONEncode(BNUserButtons))
				end
			end)
		end
...
cmd.add({"addalias"}, {"addalias <command> <alias>", "Adds a persistent alias for an existing command"}, function(original, alias)
	...
	cmds.BNSAVEDALIASES[alias] = true

	if FileSupport then
		local aliasMap = BNmanage.readAliasFile()
		aliasMap[alias] = original
		writefile(BNfiles.BNALIASPATH, HttpService:JSONEncode(aliasMap))
	end
...
cmd.add({"removealias"}, {"removealias", "Select and remove a saved alias"}, function()
	local aliasMap = FileSupport and BNmanage.readAliasFile() or {}
...
cmd.add({"clearaliases"}, {"clearaliases", "Removes all aliases created using addalias."}, function()
	...
	for alias in pairs(cmds.BNSAVEDALIASES) do
		cmds.Aliases[alias] = nil
	end
...
cmd.add({"addautoexec", "aaexec", "addae", "addauto", "aexecadd"}, {"addautoexec <command> [arguments] (aaexec, addae, addauto, aexecadd)", "Add a command to autoexecute"}, function(arg1, ...)
	...
	local exists = false
	for _, cmd in ipairs(BNEXECDATA.commands) do
	...
	if not exists then
		Insert(BNEXECDATA.commands, commandName)
	end

	if #args > 0 then
		local argumentString = Concat(args, " ")
		BNEXECDATA.args[commandName] = argumentString
	else
		BNEXECDATA.args[commandName] = ""
	end

	if FileSupport then
		writefile(BNfiles.BNAUTOEXECPATH, HttpService:JSONEncode(BNEXECDATA))
	end
...
cmd.add({"removeautoexec", "raexec", "removeae", "removeauto", "aexecremove"}, {"removeautoexec (raexec, removeae, removeauto, aexecremove)", "Remove a command from autoexecute"}, function()
	if #BNEXECDATA.commands == 0 then
		DoNotif("No AutoExec commands to remove", 2)
		return
	end

	local options = {}
	for i, cmdName in ipairs(BNEXECDATA.commands) do
		local args = BNEXECDATA.args[cmdName]
		local display = args and args ~= "" and (cmdName.." "..args) or cmdName
		Insert(options, {
			Text = display,
			Callback = function()
				local removedCommand = table.remove(BNEXECDATA.commands, i)
				BNEXECDATA.args[removedCommand] = nil
...
cmd.add({"executor","exec"},{"executor (exec)","Very simple executor"},function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/main/BNexecutor.lua"))()
end)
...
cmd.add({"scripthub","hub"},{"scripthub (hub)","Thanks to scriptblox/rscripts API"},function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/main/ScriptHubBN.lua"))()
end)
...
cmd.add({"uiscale", "uscale", "guiscale", "gscale"}, {"uiscale (uscale)", "Adjust the scale of the "..adminName.." UI"}, function()
	...
	progress.Size = UDim2.new((BNUIScale - minSize) / (maxSize - minSize), 0, 1, 0)
	...
	knob.Position = UDim2.new((BNUIScale - minSize) / (maxSize - minSize), 0, -0.25, 0)
	knob.Text = ""
	...
	label.Text = "Scale: "..Format("%.2f", BNUIScale)
		...
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					dragging = false
					if FileSupport then
						writefile(BNfiles.BNUISIZEPATH, tostring(BNUIScale))
					end
				end
			end)
		end
	...
	UserInputService.InputChanged:Connect(function(input)
		if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
			...
			BNUIScale = math.clamp(newScale, minSize, maxSize)
			update(BNUIScale)
		end
	end)
...
cmd.add({"saveprefix"}, {"saveprefix <symbol>", "Saves the prefix to a file and applies it"}, function(...)
	...
	elseif newPrefix:match("&amp;") or newPrefix:match("&lt;") or newPrefix:match("&gt;") or newPrefix:match("&quot;") or newPrefix:match("&#x27;") or newPrefix:match("&#x60;") then
		...
		writefile(BNfiles.BNPREFIXPATH, newPrefix)
		opt.prefix = newPrefix
...
BNmanage.doWindows = function(position, Size, defaultText)
...
cmd.add({"ping"}, {"ping", "Shows your ping"}, function()
	...
	local guiElements = BNmanage.doWindows(UDim2.new(0.445, 0, 0, 0), UDim2.new(0, 201, 0, 35), "Ping: --")
...
cmd.add({"fps"}, {"fps", "Shows your fps"}, function()
	...
	local guiElements = BNmanage.doWindows(UDim2.new(0.445, 0, 0, 0), UDim2.new(0, 201, 0, 35), "FPS: --")
	setupClose(guiElements)
	setupDraggable(guiElements)
	local frames = {}
	local lastUpdate = 0
...
cmd.add({"stats"}, {"stats", "Shows both FPS and ping"}, function()
	...
	local guiElements = BNmanage.doWindows(UDim2.new(0.445, 0, 0, 0), UDim2.new(0, 250, 0, 35), "Ping: -- ms | FPS: --")
	setupClose(guiElements)
	setupDraggable(guiElements)
	local frames = {}
	local lastUpdate = 0
...
cmd.add({"commandcount","cc"},{"commandcount (cc)","Counds how many commands BN has"},function()
...
cmd.add({"walkfling", "wfling", "wf"}, {"walkfling (wfling,wf)", "probably the best fling lol"}, function()
	...
	if not opt.BN_storage:FindFirstChild("juisdfj0i32i0eidsuf0iok") then
		local detection = InstanceNew("Decal")
		detection.Name = "juisdfj0i32i0eidsuf0iok"
		detection.Parent = opt.BN_storage
	end
...
zmxcnsaodakscn = nil
repeat
	local UNNBNMEEDD, FUFRRRR = pcall(function()
...
cmd.add({"uptime"}, {"uptime", "Shows how long the game/session has been running"}, function()
	local uptime = os.clock() - BNSESSIONSTARTEDIDK
...
cmd.add({"oldroblox"},{"oldroblox","Old skybox and studs"},function()
	for i,v in pairs(workspace:GetDescendants()) do
		if v:IsA("BasePart") then
			...
			dec.Texture=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.Stud) or "rbxassetid://48715260"
			dec.Face="Top"
			dec.StudsPerTileU="1"
			dec.StudsPerTileV="1"
			...
			dec2.Texture=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.Inlet) or "rbxassetid://20299774"
			dec2.Face="Bottom"
	...
	sky.SkyboxBk=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.bk) or "rbxassetid://161781263"
	sky.SkyboxDn=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.dn) or "rbxassetid://161781258"
	sky.SkyboxFt=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.ft) or "rbxassetid://161781261"
	sky.SkyboxLf=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.lf) or "rbxassetid://161781267"
	sky.SkyboxRt=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.rt) or "rbxassetid://161781268"
	sky.SkyboxUp=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.up) or "rbxassetid://161781260"
end)
...
cmd.add({"Decompiler"},{"Decompiler","Allows you to decompile LocalScript/ModuleScript's"},function()
	Spawn(function()
		...
		function call(konstantType: string, scriptPath: Script | ModuleScript | LocalScript): string
			...
			local httpResult = opt.BNREQUEST({
...
cmd.add({"functionspy"},{"functionspy","Check console"},function()
	...
	clear.ZIndex=2
	clear.Image=getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.Sheet) or "rbxassetid://3926305904"
	...
	shadow.Image = getcustomasset and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.blur) or "rbxassetid://1316045217"
...
cmd.add({"badgeviewer", "badgeview", "bviewer","badgev","bv"},{"badgeviewer (badgeview, bviewer, badgev, bv)","loads up a badge viewer UI that views all badges in the game you're in"},function()
	...
	local function getBadges()
		...
		repeat
			...
			local res = opt.BNREQUEST({Url = url, Method = "GET"})
...
cmd.add({"notepad"},{"notepad","notepad for making scripts / etc"},function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/ltseverydayyou/Nameless-Admin/main/BNnotepad.lua"))()
end)
...
cmd.add({"keepna"}, {"keepna", "keep executing "..adminName.." every time you teleport"}, function()
	BNQoTEnabled=true
	if FileSupport then
		writefile(BNfiles.BNQOTPATH, "true")
...
--[[ GUI VARIABLES ]]--
repeat
	local BNSUC, resexy = pcall(function()
		return loadstring(game:HttpGet(opt.BNUILOADER))()
	end)
...
if not RunService:IsStudio() then
	...
	BNSCREENGUI=player:FindFirstChild("AdminUI",true)
end
--repeat Wait() until ScreenGui~=nil -- if it loads late then I'll just add this here

NaProtectUI(BNSCREENGUI)

local description = BNSCREENGUI:FindFirstChild("Description")
local AUTOSCALER = BNSCREENGUI:FindFirstChild("AutoScale")
local cmdBar = BNSCREENGUI:FindFirstChild("CmdBar")
...
local chatLogsFrame = BNSCREENGUI:FindFirstChild("ChatLogs")
...
local BNconsoleFrame = BNSCREENGUI:FindFirstChild("soRealConsole")
local BNconsoleLogs = BNconsoleFrame and BNconsoleFrame:FindFirstChild("Container") and BNconsoleFrame:FindFirstChild("Container"):FindFirstChild("Logs")
local BNconsoleExample = BNconsoleLogs and BNconsoleLogs:FindFirstChildWhichIsA("TextLabel")
local BNcontainer = BNconsoleFrame and BNconsoleFrame:FindFirstChild("Container")
local BNfilter = BNcontainer and BNcontainer:FindFirstChild("Filter")
local commandsFrame = BNSCREENGUI:FindFirstChild("Commands")
...
local resizeFrame = BNSCREENGUI:FindFirstChild("Resizeable")
local ModalFixer = BNSCREENGUI:FindFirstChildWhichIsA("ImageButton")
local SettingsFrame = BNSCREENGUI:FindFirstChild("setsettings")
...
if BNconsoleExample then
	BNconsoleExample.Parent = nil
end
...
Spawn(function()
	for _,v in ipairs(BNSCREENGUI:GetDescendants()) do
		if v:IsA("UIStroke") then
			Insert(BNCOLOREDELEMENTS, v)
		end
	end
end)
...
opt.BNAUTOSCALER = AUTOSCALER

	--[[pcall(function()
		for i,v in pairs(BNSCREENGUI:GetDescendants()) do
			coreGuiProtection[v]=rPlayer.Name
		end
		BNSCREENGUI.DescendantAdded:Connect(function(v)
			coreGuiProtection[v]=rPlayer.Name
		end)
		coreGuiProtection[BNSCREENGUI]=rPlayer.Name
	...
	if not RunService:IsStudio() then
		local newGui=COREGUI:FindFirstChildWhichIsA("BNSCREENGUI")
		newGui.DescendantAdded:Connect(function(v)
			coreGuiProtection[v]=rPlayer.Name
		end)
		for i,v in pairs(BNSCREENGUI:GetChildren()) do
			v.Parent=newGui
		end
		BNSCREENGUI=newGui
	end]]
...
cmd.add({"unname"}, {"unname", "Resets the admin UI placeholder name to default"}, function(...)
	adminName = getgenv().BNTestingVer and "BN Testing" or "Nameless Admin"
	if cmdInput and cmdInput.PlaceholderText then
...
gui.consoleeee = function()
	if BNconsoleFrame then
		if not BNconsoleFrame.Visible then
			BNconsoleFrame.Visible = true
		end
		BNconsoleFrame.Position = UDim2.new(0.43, 0, 0.4, 0)
	end
end
...
Spawn(function() -- plugin tester
	while Wait(2) do
		if countDictBN(cmds.Commands) ~= cmdBNnum then
			gui.loadCMDS()
		end
	end
end)
...
gui.searchCommands = function()
	...
	searchHeartbeat = RunService.Heartbeat:Connect(function()
		...
		for _, frame in ipairs(CMDAUTOFILL) do
			...
			elseif cmds.BNSAVEDALIASES[searchTerm] and cmds.BNSAVEDALIASES[searchTerm] == cmdName then
				score = 3
				matchText = searchTerm
			else
				...
				for alias, realCmd in pairs(cmds.BNSAVEDALIASES) do
...
if BNconsoleFrame then gui.resizeable(BNconsoleFrame) end
...
function bindToChat(plr, msg)
	...
	local isBNadmin = false
	if _G.BNadminsLol then
		for _, id in ipairs(_G.BNadminsLol) do
...
BNmanage.bindToDevConsole = function()
	if not BNconsoleLogs or not BNconsoleExample then return end
	...
	FilterButtons.Parent = BNconsoleLogs.Parent
	...
	for _, logType in ipairs(buttonTypes) do
		...
		MouseButtonFix(clickZone,function()
			...
			for _, label in pairs(BNconsoleLogs:GetChildren()) do
				if label:IsA("TextLabel") and label:FindFirstChild("Tag") then
					local tag = label.Tag.Value
					local matchesSearch = BNfilter.Text == "" or Find(label.Text:lower(), BNfilter.Text:lower())
	...
	BNfilter:GetPropertyChangedSignal("Text"):Connect(function()
		local query = BNfilter.Text:lower()
		for _, label in pairs(BNconsoleLogs:GetChildren()) do
			if label:IsA("TextLabel") and label:FindFirstChild("Tag") then
				local tag = label.Tag.Value
	...
	SafeGetService("LogService").MessageOut:Connect(function(msg, msgTYPE)
		...
		local logLabel = BNconsoleExample:Clone()
		logLabel.Name = "Log_"..tostring(math.random(100000, 999999))
		logLabel.Parent = BNconsoleLogs
		...
		for _, v in pairs(BNconsoleLogs:GetChildren()) do
			if v:IsA("TextLabel") then
				Insert(logFrames, v)
			end
		end
		...
		local matchesSearch = BNfilter.Text == "" or Find(logLabel.Text:lower(), BNfilter.Text:lower())
		logLabel.Visible = toggles[tagText] and matchesSearch
	end)
end

--[[function BNUISCALEUPD()
...
Players.PlayerRemoving:Connect(function(plr)
	...
	if JoinLeaveConfig.LeaveLog then
		...
		BNmanage.LogJoinLeave(leaveMsg)
	end
end)
...
RunService.RenderStepped:Connect(function()
	...
	if BNconsoleLogs then updateCanvasSize(BNconsoleLogs, AUTOSCALER.Scale) end
...
RunService.RenderStepped:Connect(function()
	...
	if isInvalid(p) then
		if opt.prefix ~= ";" then
			...
			if FileSupport and isfile(BNfiles.BNPREFIXPATH) then
				local filePrefix = readfile(BNfiles.BNPREFIXPATH)
...
--RunService.RenderStepped:Connect(BNUISCALEUPD)

BNCaller(function()
	if BNjson.annc and BNjson.annc ~= "" then
		DoWindow(BNjson.annc, 99999999999999999, adminName.." Announcement")
	end
end)
...
BNCaller(function() BNICOBNSSET=(getcustomasset and (isAprilFools() and getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.sWare) or getcustomasset(BNfiles.BNASSETSFILEPATH.."/"..BNImageAssets.Icon))) or nil end)

if BNICOBNSSET then
	TextButton = InstanceNew("ImageButton")
	TextButton.Image = BNICOBNSSET
else
	...
	if isAprilFools() then
		cringyahhnamesidk = { "IY", "FE", "F3X", "HD", "CMD", "Ω", "R6", "Ø", "BN", "CMDX" }
...
UIStroke.Thickness = 2
UIStroke.Color = BNUISTROKER --Color3.fromRGB(148, 93, 255)
UIStroke.Transparency = 0.4
UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border

TextButton.Parent = BNSCREENGUI
...
TextButton.Size = UDim2.new(0, 32 * BNScale, 0, 32 * BNScale)
...
TextButton.MouseEnter:Connect(function()
	TweenService:Create(TextButton, TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
		Size = UDim2.new(0, 35 * BNScale, 0, 35 * BNScale)
	}):Play()
end)

TextButton.MouseLeave:Connect(function()
	TweenService:Create(TextButton, TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
		Size = UDim2.new(0, 32 * BNScale, 0, 32 * BNScale)
	}):Play()
end)
...
function Swoosh()
	...
	TextButton.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					if FileSupport and BNiconSaveEnabled then
						local pos = TextButton.Position
						writefile(BNfiles.BNICONPOSPATH, HttpService:JSONEncode({
							X = pos.X.Scale,
							Y = pos.Y.Scale,
							Save = BNiconSaveEnabled
						}))
					end
				end
			end)
...
function mainNameless()
	...
	if FileSupport and isfile(BNfiles.BNICONPOSPATH) then
		local data = HttpService:JSONDecode(readfile(BNfiles.BNICONPOSPATH))
...
Spawn(function()
	...
	Delay(0.3, function()
		...
		local notifBody = welcomeMessage..
			...
			"\nUpdated on: "..opt.BNupdDate..
			"\nTime Taken To Load: "..loadedResults(BNresult)

		DoNotif(notifBody, 6, rngMsg().." "..nameCheck)

		if not FileSupport then
			warn("BNWWW NO FILE SUPPORT???????")
			Window({
	...
	Spawn(function()
		Wait(.5)
		for _, commandName in ipairs(BNEXECDATA.commands) do
			local fullRun = {commandName}
			local argsString = BNEXECDATA.args[commandName]
...
TextChatService.OnIncomingMessage = function(message)
	...
	for _, adminId in ipairs(_G.BNadminsLol or {}) do
		if fromPlayer.UserId == adminId then
			...
			props.PrefixText = Format('<font color="%s">[BN ADMIN]</font> %s', hex, message.PrefixText or "")
			props.Text = message.Text
			return props
		end
	end

	local tagText = fromPlayer:GetAttribute("CustomBNtaggerText")
	local tagColor = fromPlayer:GetAttribute("CustomBNtaggerColor")
...
Spawn(function() -- init
	if cmdBar then BNProtection(cmdBar) end
	if chatLogsFrame then BNProtection(chatLogsFrame) end
	if BNconsoleFrame then BNProtection(BNconsoleFrame) end
	if commandsFrame then BNProtection(commandsFrame) end
	if resizeFrame then BNProtection(resizeFrame) end
	if description then BNProtection(description) end
	if ModalFixer then BNProtection(ModalFixer) end
	if AUTOSCALER then BNProtection(AUTOSCALER) AUTOSCALER.Scale = BNUIScale end
	if SettingsFrame then BNProtection(SettingsFrame) end
end)

Spawn(BNmanage.bindToDevConsole)
Spawn(BNmanage.loadAliases)
Spawn(BNmanage.loadButtonIDS)
Spawn(BNmanage.RenderUserButtons)
Spawn(BNmanage.loadAutoExec)
Spawn(BNmanage.LoadPlugins)
...
if FileSupport then
	gui.addButton("Save Prefix", function()
		if isfile(BNfiles.BNPREFIXPATH) then
			writefile(BNfiles.BNPREFIXPATH, opt.prefix)
			DoNotif("Prefix saved to file: "..BNfiles.BNPREFIXPATH)
		else
			DoNotif("File not found: "..BNfiles.BNPREFIXPATH)
		end
	end)
end
...
gui.addToggle("Keep "..adminName, BNQoTEnabled, function(val)
	BNQoTEnabled = val
	if FileSupport then
		writefile(BNfiles.BNQOTPATH, tostring(val))
	end
	if BNQoTEnabled then
...
gui.addToggle("Command Predictions Prompt", doPREDICTION, function(v)
	...
	if FileSupport then
		writefile(BNfiles.BNPREDICTIONPATH, tostring(v))
	end
end)

gui.addToggle("Keep Icon Position", BNiconSaveEnabled, function(v)
	local pos = TextButton.Position
	writefile(BNfiles.BNICONPOSPATH, HttpService:JSONEncode({
...
if FileSupport then
	...
	gui.addToggle("Log Player Joins", JoinLeaveConfig.JoinLog, function(v)
		...
		writefile(BNfiles.BNJOINLEAVE, HttpService:JSONEncode(JoinLeaveConfig))
...
gui.addSlider("BN Icon Size", 0.5, 3, BNScale, 0.01, "", function(val)
	BNScale = val
	TextButton.Size = UDim2.new(0, 32 * val, 0, 33 * val)
	if FileSupport then
		writefile(BNfiles.BNBUTTONSIZEPATH, tostring(val))
	end
end)

gui.addColorPicker("UI Stroke", BNUISTROKER, function(color)
	for _, element in ipairs(BNCOLOREDELEMENTS) do
		if element:IsA("UIStroke") then
			element.Color = color
		end
	end
	SaveUIStroke(BNfiles.BNSTROKETHINGY, color)
end)
...
gui.addButton("Apply Chat Tag", function()
	...
	LocalPlayer:SetAttribute("CustomBNtaggerText", opt.currentTagText)
	LocalPlayer:SetAttribute("CustomBNtaggerColor", opt.currentTagColor)
...
gui.addButton("Remove Chat Tag", function()
	LocalPlayer:SetAttribute("CustomBNtaggerText", nil)
	LocalPlayer:SetAttribute("CustomBNtaggerColor", nil)

	if FileSupport and isfile(BNfiles.BNCHATTAG) then
		writefile(BNfiles.BNCHATTAG, HttpService:JSONEncode({
			Text = opt.currentTagText;
