...
NACaller(function() while not NAjson do Wait(.1) NAjson=HttpService:JSONDecode(game:HttpGet("https://raw.githubusercontent.com/Freeman4i37/Freeman-scripts/refs/heads/main/stuff.json")) end end)
...
if not game:IsLoaded() then
	...
	message.Text = adminName.." waiting for the game to load!"
	NaProtectUI(message)
	game.Loaded:Wait()
...
repeat
	local success, result = pcall(function()
		return loadstring(game:HttpGet("https://raw.githubusercontent.com/Freeman4i37/bandu-admin/main/notifications.lua"))()
	end)
...
if FileSupport then
	...
	if prefixCheck == "" or utf8.len(prefixCheck) > 1 or prefixCheck:match("[%w]")
		...
		prefixCheck = ";"
		writefile(NAfiles.NAPREFIXPATH, ";")
		DoNotif("Your PREFIX has been reset to (;) because you entered an invalid symbol.")
	end
	...
	if NAsavedScale and NAsavedScale > 0 then
		...
		NAScale = 1
		writefile(NAfiles.NABUTTONSIZEPATH, "1")
		DoNotif("ImageButton size was reset to default due to data error.")
	end
	...
	if NAUISavedScale and NAUISavedScale > 0 then
		...
		NAUIScale = 1
		writefile(NAfiles.NAUISIZEPATH, "1")
		DoNotif("UIScale has been reset to default due to invalid data.")
	end
	...
	if isfile(NAfiles.NACHATTAG) then
		...
		if success and typeof(data) == "table" then
			...
			opt.saveTag = false
			DoNotif("ChatTAG gave error or not readable.",3)
		end
	end
	...
	opt.currentTagColor = Color3.fromRGB(0, 255, 170)
	opt.saveTag = false
	DoWindow("Your exploit/app programmer does not support Bandu's Admin.")
end
...
cmd.add({"scripthub","hub"},{"scripthub (hub)","NIL"},function()
...
cmd.add({"pwalkfling", "pwfling", "pwf"}, {"pwalkfling (pwfling,pwf)", "probably the best fling lol"}, function()
	if hiddenfling then return end

	DoNotif("POWERFULL WALKFLING enabled", 2)
	hiddenfling = true
	...
	lib.disconnect("pwalkflinger")
	lib.connect("pwalkflinger", RunService.Heartbeat:Connect(function()
		if not hiddenfling then return end
	...
	local lp = Players.LocalPlayer
	if lp then
		lib.disconnect("pwalkfling_charfix")
		lib.connect("pwalkfling_charfix", lp.CharacterAdded:Connect(function()
			if hiddenfling then
				DoNotif("Re-enabling Walkfling")
			end
		end))
...
cmd.add({"tinywalkfling", "twfling", "twf"}, {"walkfling (wfling,wf)", "probably the best fling lol"}, function()
	if hiddenfling then return end

	DoNotif("Tiny Walkfling enabled", 2)
	hiddenfling = true
	...
	lib.disconnect("twalkflinger")
	lib.connect("twalkflinger", RunService.Heartbeat:Connect(function()
		if not hiddenfling then return end
		...
		if character and hrp then
			local originalVelocity = hrp.Velocity
			hrp.Velocity = originalVelocity * 100 + Vector3.new(0, 100, 0)
	...
	local lp = Players.LocalPlayer
	if lp then
		lib.disconnect("twalkfling_charfix")
		lib.connect("twalkfling_charfix", lp.CharacterAdded:Connect(function()
			if hiddenfling then
				DoNotif("Re-enabling TWalkfling")
			end
		end))
	end
end)
...
cmd.add({"punwalkfling", "punwfling", "punwf"}, {"punwalkfling (punwfling,punwf)", "stop the walkfling command"}, function()
	if not hiddenfling then return end
...
cmd.add({"untwalkfling", "untwfling", "untwf"}, {"untwalkfling (untwfling,untwf)", "stop the walkfling command"}, function()
	if not hiddenfling then return end

	DoNotif("Twalkfling disabled", 2)
	hiddenfling = false
...
cmd.add({"admin"},{"admin","whitelist someone to allow them to use commands"},function(...)
	...
	for _, plr in next, Player do
		if plr~=nil and not Admin[plr.UserId] then
			...
			ChatMessage("["Bandu's Admin"] The Owner give you admin, CONTROL HIM! Prefix: ';'",plr.Name)
			Wait(0.2)
...
cmd.add({"unadmin"},{"unadmin <player>","removes someone from being admin"},function(...)
	...
	for _, plr in next, Player do
		if plr~=nil and Admin[plr.UserId] then
			...
			ChatMessage("Unfortunately you can't control me anymore.",plr.Name)
	...
	if IsR6() then
		...
		lib.connect("hug_toggle", MouseButtonFix(toggleHugButton, function()
			...
			if hugModeEnabled then
				toggleHugButton.Text = "ON"
			else
				toggleHugButton.Text = "OFF"
				for _, track in pairs(currentHugTracks) do pcall(function() track:Stop() end) end
				currentHugTracks = {}
				currentHugTarget = nil
				for _, part in pairs(huggiePARTS) do part:Destroy() end
		...
		lib.connect("hug_side", MouseButtonFix(sideToggleButton, function()
			...
			sideToggleButton.Text = (hugFromFront and "FRONT") or "BACK"
		end))
...
cmd.add({"hug15", "clickhug15"}, {"hug (clickhug)", "huggies time (click on a target to hug)"}, function()
	if IsR15() then
		local mouse = LocalPlayer:GetMouse()
...
cmd.add({"realflingnpcs"},{"realflingnpcs","Flings NPCs"},function()
	local npcs={}
	local function fling(hum)
		if hum:IsA("Humanoid")and not Players:GetPlayerFromCharacter(hum.Parent)then
			local hrp=hum.Parent:FindFirstChild("HumanoidRootPart")
			if hrp then
				table.insert(npcs,{hum,hum.HipHeight})
				hrp.Velocity=Vector3.new(math.random(-999999,999999),math.random(500000,999999),math.random(-999999,999999))
				hum.HipHeight=1024
			end
		end
	end
...
print([[
	BANDU'S ADMIN: Welcome to the best admin-program!
]])
